...
export default function MembershipValidationPage() {
  ...
  const handleStatusChange = async (applicationId: string, status: ApplicationStatus, rejectionReason?: string) => {
    try {
      ...
      if (response.ok) {
        // Если одобрено, отправляем письмо
        if (status === 'APPROVED' && selectedApplication) {
          try {
            await fetch('/api/membership/approval-email', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({
                ...
                organizationName: selectedApplication.organization.name,
              }),
            });
          } catch (emailError) {
        ...
        setNotification({ 
          message: status === 'APPROVED' 
            ? '✓ Членство успешно одобрено! Письмо отправлено члену.' 
            : '✗ Заявление отклонено.', 
          type: 'success' 
        });
      } else {
        const errorData = await response.json();
        setNotification({ message: errorData.message || 'Ошибка при обновлении статуса заявления.', type: 'error' });
      }
    } catch (error) {
      console.error('Error updating application status:', error);
      setNotification({ message: 'Произошла ошибка при обновлении статуса заявления.', type: 'error' });
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8 bg-gray-900 min-h-screen">
                ...
                {filteredApplications.map((application) => (
                  <tr key={application.id} className="hover:bg-gray-700/50">
                    <td className="px-3 py-3">
                      <div className="text-xs font-medium text-white max-w-xs truncate">
                        {application.firstName} {application.lastName}
                      </div>
                      <div className="text-xs text-gray-400 truncate">{application.phone}</div>
                      {application.user?.email && (
                        <div className="text-xs text-gray-400 truncate">{application.user.email}</div>
                      )}
                    </td>
                    <td className="px-3 py-3">
                      <div className="text-xs text-white truncate max-w-xs">{application.organization.name}</div>
                    </td>
                    <td className="px-3 py-3 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full inline-block ${getStatusColor(application.status)}`}>
                    ...
                    <td className="px-3 py-3 whitespace-nowrap text-center">
                      <button
                        ...
                        className="p-1 text-blue-400 hover:text-blue-300 hover:bg-blue-900/20 rounded-lg transition-colors"
        ...
        {/* Notification */}
        {notification && (
          <div className={`fixed bottom-4 right-4 px-6 py-3 rounded-lg shadow-lg animate-in fade-in slide-in-from-bottom-4 ${
